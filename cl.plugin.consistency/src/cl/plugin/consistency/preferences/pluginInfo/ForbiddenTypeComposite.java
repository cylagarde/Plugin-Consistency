...
class ForbiddenTypeComposite
{
   ...
   * @param style
   */
  ForbiddenTypeComposite(ProjectDetail projectDetail, Composite parent, int style)
  {
  ...
  private List<String> getNotUsedTypes()
  {
    ...
    types.add(0, "");
    List<String> forbiddenTypes = pluginInfo.forbiddenTypeList.stream().map(forbiddenType -> forbiddenType.name).collect(Collectors.toList());
    ...
    public void accept(IStructuredSelection oldStructuredSelection, IStructuredSelection newStructuredSelection)
    {
      ...
      if (selectedTypeName == null || selectedTypeName.isEmpty())
      {
        if (forbiddenTypeIndex < pluginInfo.forbiddenTypeList.size())
          pluginInfo.forbiddenTypeList.remove(forbiddenTypeIndex);
