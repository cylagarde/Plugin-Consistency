...
/**
 * The class <b>PatternForbiddenTypeComposite</b> allows to.<br>
 */
public class PatternForbiddenTypeComposite
{
   ...
   * @param style
   */
  public PatternForbiddenTypeComposite(PatternTabItem patternTabItem, Composite parent, int style)
  {
    this.patternTabItem = patternTabItem;

    FormToolkit toolkit = new FormToolkit(parent.getDisplay());
    section = toolkit.createSection(parent, Section.TITLE_BAR | Section.EXPANDED);
    section.setText("Forbidden types");
  ...
  private List<String> getNotUsedTypes()
  {
    ...
    types.add(0, "");
    List<String> forbiddenTypes = patternInfo.forbiddenTypeList.stream().map(forbiddenType -> forbiddenType.name).collect(Collectors.toList());
  ...
  class TypeBiConsumer implements BiConsumer<IStructuredSelection, IStructuredSelection>
  {
    ...
    public void accept(IStructuredSelection oldStructuredSelection, IStructuredSelection newStructuredSelection)
    {
      ...
      {
        if (forbiddenTypeIndex < patternInfo.forbiddenTypeList.size())
        {
          patternInfo.forbiddenTypeList.remove(forbiddenTypeIndex);
        }
      ...
      else
      {
        ...
        {
          // same -> no change
          Type type = patternInfo.forbiddenTypeList.get(forbiddenTypeIndex);
        ...
        else
        {
          Type type = new Type();
          type.name = selectedTypeName;
          patternInfo.forbiddenTypeList.add(type);
        }
      }
  ...
  {
    {
      ...
      setToolTipText("Add forbidden type");
    }
